...
class BurgState extends FlxState
{
  ...
  override public function create():Void {
    ...
    if (Reg.unlocks.scrying) {
      scryButton = new GradientButton(FlxG.width/2 - 310, 460, 620, 100, "Scrying Pool");
      scryButton.onUp.callback = function():Void {
        Transition.to(new ScryState());
      }
      add(scryButton);
    }

    if (Reg.unlocks.cauldron) {
      cauldronButton = new GradientButton(FlxG.width/2 - 310, 580, 620, 100, "Witch Queen's Cauldron");
